...
class TCPDF {
	...
	protected $footer_logo_width = 40;	
	 ...
	 * @public
	 */
	public function setFooterData($ln='', $lw=0, $ht='', $hs='', $tc=array(0,0,0), $lc=array(0,0,0)) {
		$this->footer_logo = $ln;
	...
	public function getFooterData() {
		...
		$ret['logo_width'] = $this->footer_logo_width;
		$ret['title'] = $this->footer_title;
		$ret['string'] = $this->footer_string;
		$ret['text_color'] = $this->footer_text_color;
		$ret['line_color'] = $this->footer_line_color;
		return $ret;
	}	
...
//		$image_file = "images/print_footer.png";

		$footerdata = $this->getFooterData();
		if (($footerdata['logo']) AND ($footerdata['logo'] != K_BLANK_IMAGE)) {
			$imgtype = TCPDF_IMAGES::getImageFileType(K_PATH_IMAGES.$footerdata['logo']);
			if (($imgtype == 'eps') OR ($imgtype == 'ai')) {
				$this->ImageEps(K_PATH_IMAGES.$footerdata['logo'], '', '', $footerdata['logo_width']);
			} elseif ($imgtype == 'svg') {
				$this->ImageSVG(K_PATH_IMAGES.$footerdata['logo'], '', '', $footerdata['logo_width']);
			} else {
				$this->Image(K_PATH_IMAGES.$footerdata['logo'], '', '', $footerdata['logo_width']);
			}
			$imgy = $this->getImageRBY();
		} else {
			$imgy = $this->y;
...
/*		$this->SetY($cur_y);
		...
		if ($this->getRTL()) {
			$this->SetX($this->original_rMargin);
//		    $this->Image($image_file, 0, 0, 210, "", "PNG");
