...
function create_payment() {
  ...
  var item_flag =  '';
   ...
   $(".tf4").each(function() {
    ...
    var tax = $("#cmb_tax").val();
     var tax_type = $("#cmb_type").val();
     var tax_app = $("#chk_tax_app:checked").val();
  ...
  if(payment_type == ''){
    ...
    return false;
  }else if(tax_app == 1 && tax_type == '' && tax == 0){
    alert("Please Select Tax properly");
     $("#cmb_account_group").focus();
    return false;
  } else if(flag == 'empty') {
...
<div class="container category">        
  <div class="row">
    <ol class = "breadcrumb">                
        <li style="color: red;"><b>EDIT PAYMENT VOUCHER</b></li> 
    ...
    <div class="col-md-12" style="background-color:#FFFACD;border-radius: 5px 5px 5px 5px; border: solid 1px #000000; padding:8px 8px 8px 8px;">
            <form name="frm_debit" id="frm_debit" action="<?php echo site_url('entry/update_payment_voucher');?>" method="post" >
        <div class="textdata_myaccount">
                ...
                <label for="emirate" class="col-md-12 control-label">Voucher No<span class="textspandata">*</span></label>
                    ...
                    <?php 
                        ...
                        <select id="cmb_payment_type1"  class="input-sm" name="cmb_payment_type1" style="width:35%;    border: 1px solid #555555 !important; background-color: #f5f5f5; font-size: 15px;" disabled="true" >
                          ...
                          <option value="CP" <?php echo $selected_cr;?>>Cash Payment</option>
                          <option value="BP" <?php echo $selected_br;?>>Bank Payment</option>                         
                        </select>                 
                        <input type="text" id="cmb_voucher_type"  class="input-sm" name="cmb_voucher_type" style="width:25%;    border: 1px solid #555555 !important; background-color: #f5f5f5; font-size: 15px;" value="<?php echo $prefix;?>" readonly>
          ...
          <tr class="to_clone1">
          <td>
            <select id="cmbledger_<?php echo $j;?>" name="cmb_ledger[]" class="tf4 form-control input-sm"  style="font-size: 16px;">
                        <option selected="selected" value="">select Ledger</option>
                          <?php foreach ($ledger as $row) {
                             if($value['DR_ACCOUNT_ID'] == $row['ID']){
                              echo "<option value='".$row['ID']."' selected>".$row['ACCOUNTDESC']."</option>";
                             }else{
                              echo "<option value='".$row['ID']."'>".$row['ACCOUNTDESC']."</option>";
                             }
          ...
          <td>
              <input name="txt_total[]" type="number" class="tf14 addeb_input_txtfld input_grid" id="txttotal_<?php echo $j;?>"  maxlength = "10" min = "0"  oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" value="<?php echo $value['AMOUNT'];?>" style="font-size: 16px;"/>
          </td>
     ...
     <div class="col-md-12 col-sm-12 col-xs-12">               
            ...
            <div style="height:5px;"></div>
                  <div class="text-center">
                    ...
                    <input type="reset" class="btn btn-primary" name="reset" id="reset" value="Reset" />&nbsp;&nbsp;<input type="button" class="btn btn-primary" name="reset" id="reset" value="Cancel" onclick="window.location.href='<?php echo site_url('entry/manage_payment_voucher');?>'" />&nbsp;&nbsp;<!--<input type="button" class="btn btn-primary" name="btn_delete" id="btn_delete" value="Delete"  onclick=" if (confirm('Do you want delete this Payment?')){window.location.href='<?php echo site_url('/entry/delete_payment_voucher/'.$sn_id.'/'.'/?pre='.urlencode($prefix));?>';}else{event.stopPropagation(); event.preventDefault();}; "/> -->
                </div>          
            </div>  
        </form>       
    </div>    
