...
if(isset($_GET['msg'])){
 ...
 // var msg = '<?php echo $_GET['msg'];?>'
  ...
  window.location='<?php echo site_url(); ?>entry/manage_creditnote/';
  <?php
}
?>
</script>
<div class="container category" style="min-height: 560px;">        
  <div class="row">
    <ol class = "breadcrumb">                
        <li style="color: red;">MANAGE CREDIT NOTE</li> 
     ...
     if($islock != 1) {
      ?>
      <a href="<?php echo site_url('/entry/credit_note');?>" alt="ADD CREDIT NOTE" title="ADD CREDIT NOTE"><img src="https://test.newui.myddf.info/images/add.png" alt="ADD CREDIT NOTE" title="ADD CREDIT NOTE"/> ADD CREDIT NOTE</a>
      <?php
     }
     ?>
     </div>   
          ...
          ?>
            <tr>
               ...
               <!-- <td><?php echo $financial_year_selected;?></td> -->
                ...
                <td><?php if($val['Printed'] == 0){?><a href="<?php echo site_url('/entry/edit_creditnote/'.$val['VOUCHER_ID'].'/'.$val['PREFIX']);?>"><span class="fa fa-pencil-square-o"></span></a><!--&nbsp;&nbsp;<a href="<?php echo site_url('/entry/delete_credit_note/'.$val['VOUCHER_ID'].'/'.'/?pre='.urlencode($val['PREFIX']));?>"  onclick="if (confirm('Delete selected item?')){return true;}else{event.stopPropagation(); event.preventDefault();};"><span class="fa fa-trash" aria-hidden="true"></span></a> --><?php } ?>&nbsp;&nbsp;<a href="<?php echo site_url('/entry/print_credit_note/'.$val['VOUCHER_ID'].'/'.$val['PREFIX']);?>"><span class="fa fa-print" aria-hidden="true"></span></a></td>
            </tr>
        <?php
         }
        ?>
