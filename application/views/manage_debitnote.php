...
if(isset($_GET['msg'])){
 ...
 // var msg = '<?php echo $_GET['msg'];?>'
  ...
  window.location='<?php echo site_url(); ?>entry/manage_debitnote/';
  <?php
}
?>
</script>
<div class="container category" style="min-height: 560px;">        
  <div class="row">
    <ol class = "breadcrumb">                
        <li style="color: red;">MANAGE DEBIT NOTE</li>   
     ...
     if($islock != 1) {
      ?>
     <a href="<?php echo site_url('/entry/debit_note');?>" alt="ADD DEBIT NOTE" title="ADD DEBIT NOTE"><img src="https://test.newui.myddf.info/images/add.png" alt="ADD DEBIT NOTE" title="ADD DEBIT NOTE"/> ADD DEBIT NOTE</a>
     <?php
     }
     ?>
     </div>   
<table id="myTable" class="table table-striped table-bordered" cellspacing="0" width="100%">
        ...
        ?>
            <tr>
                ...
                <td><?php if($val['Printed'] == 0){?><a href="<?php echo site_url('/entry/edit_debitnote/'.$val['VOUCHER_ID'].'/'.$val['PREFIX']);?>"><span class="fa fa-pencil-square-o"></span></a><!--&nbsp;&nbsp;<a href="<?php echo site_url('/entry/delete_debit_note/'.$val['VOUCHER_ID'].'/'.'/?pre='.urlencode($val['PREFIX']));?>"  onclick="if (confirm('Delete selected item?')){return true;}else{event.stopPropagation(); event.preventDefault();};"><span class="fa fa-trash" aria-hidden="true"></span></a>--><?php } ?>&nbsp;&nbsp;<a href="<?php echo site_url('/entry/print_debit_note/'.$val['VOUCHER_ID'].'/'.$val['PREFIX']);?>"><span class="fa fa-print" aria-hidden="true"></span></a></td>
            </tr>
        <?php
         }
        ?>
